...
| リソース | 値 | 状態 |
|----------|-----|------|
| EC2 インスタンス | `i-08347ed1e2c4bcc80` | ✅ 稼働中 |
| Private IP | `10.0.0.203` | ✅ |
| FastAPI | `http://10.0.0.203:8000` | ✅ 稼働中 |
| VPC | `vpc-0c81b6d3f4cf43e66` | ✅ |
| セキュリティグループ | `sg-07be5462977348b46` | ✅ |

### データ同期状況

| 項目 | 値 |
|------|-----|
| 最終同期日時 | 2026-01-21 01:47:51 |
| 最終タイムスタンプ | 20260119133706 |
| レコード数 | 1,168 |
| 同期済み日付 | 2026-01-17, 2026-01-18 |

### CDKスタック

```bash
# JRA-VAN連携モードでデプロイ
npx cdk deploy BakenKaigiApiStack --context jravan=true
```
...
| 変数名 | 値 |
|--------|-----|
| RACE_DATA_PROVIDER | `jravan` |
| JRAVAN_API_URL | `http://10.0.0.203:8000` |

### 注意事項

- JV-Link同期はSSM経由では動作しません（GUI操作が必要）
- 新しいデータを取得するには、EC2にRDP接続して手動で `python sync_jvlink.py` を実行
