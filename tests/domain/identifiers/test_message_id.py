"""MessageIdのテスト."""
import pytest

from src.domain.identifiers import MessageId


class TestMessageId:
    """MessageIdの単体テスト."""

    def test_値を指定して生成できる(self) -> None:
        """文字列値を指定してMessageIdを生成できることを確認."""
        message_id = MessageId("msg-123")
        assert message_id.value == "msg-123"

    def test_空文字列で生成するとエラー(self) -> None:
        """空文字列を指定するとValueErrorが発生することを確認."""
        with pytest.raises(ValueError, match="cannot be empty"):
            MessageId("")

    def test_generateで新しいIDを生成できる(self) -> None:
        """generateメソッドでUUID形式のIDが生成されることを確認."""
        message_id = MessageId.generate()
        assert message_id.value is not None
        assert len(message_id.value) == 36

    def test_文字列変換でvalueが返る(self) -> None:
        """str()でvalue属性の値が返ることを確認."""
        message_id = MessageId("my-message")
        assert str(message_id) == "my-message"

    def test_不変オブジェクトである(self) -> None:
        """MessageIdは不変（frozen）であることを確認."""
        message_id = MessageId("test")
        with pytest.raises(AttributeError):
            message_id.value = "changed"  # type: ignore
