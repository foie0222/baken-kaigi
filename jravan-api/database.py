...
def get_past_race_statistics(
    ...
    try:
        with get_db() as conn:
            ...
            # CTEとUNION ALLを使用して全レースの全複勝配当を個別に平均化
            payout_query = """
            ...
            payout_query += """
                ...
                all_place_payouts AS (
                    SELECT NULLIF(hr.fukusho_haraimodoshi_1, '')::numeric / 10 AS place_payout
                    FROM jvd_hr hr
                    INNER JOIN target_races tr
                        ON hr.kaisai_nen = tr.kaisai_nen
                        AND hr.kaisai_tsukihi = tr.kaisai_tsukihi
                ...
                SELECT
                    ...
                    (SELECT AVG(place_payout) FROM all_place_payouts) AS avg_place_payout
            """
